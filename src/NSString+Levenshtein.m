...
- (CGFloat)compareWithString:(NSString *)stringB
                   matchGain:(NSInteger)gain
  ...
  NSString *mStringB =
      [stringB stringByReplacingOccurrencesOfString:@"\n" withString:@" "];
  ...
  for (NSString *tokenA in arrayA) {
    ...
    for (NSString *tokenB in arrayB) {
      smallestDistance = MIN(
          (CGFloat)
              [tokenA compareWithWord:tokenB matchGain:gain missingCost:cost],
          smallestDistance);
    }
...
- (NSInteger)compareWithWord:(NSString *)stringB
                   matchGain:(NSInteger)gain
                 missingCost:(NSInteger)cost {
  // normalize strings
  ...
  if (n++ != 0 && m++ != 0) {
    ...
    for (i = 1; i < n; i++) {
      for (j = 1; j < m; j++) {
        ...
        d[j * n + i] =
            MIN(d[(j - 1) * n + i] + 1,
                MIN(d[j * n + i - 1] + 1, d[(j - 1) * n + i - 1] + change));
      }
    }
